...
bitmap <- function(file_path){
  # read in image
  my_object <- image_read(file_path)
  # convert image
...
gray_filter <- function(rgb_vector) { #OH comment: argument name changed to rbg_vector to make it clear what the input should be
  # Extract RGB values
  ...
  return(rgb_grayscale) ##OH comment :ensure you return the output for the function to work
}
...
color_filter <- function(rgb_vector, threshold = 127) { #OH comment: changed argument bitmap to rgb filter again for clarity
  grayscale <- gray_filter(rgb_vector) #OH comment: this code repeated what was done in the gray_filter thus replaced with gray_filter()
  # Apply threshold
  if (grayscale[[1]] > threshold) { #OH comment: taking the first of the three greyscale values
    out_colour <- as.raw(c(255, 0, 0))  # Red if above threshold
  } else {
    out_colour <- as.raw(c(0, 0, 255))  # Blue if below threshold
  }
  return(out_colour) #OH comment: added a return function
}
